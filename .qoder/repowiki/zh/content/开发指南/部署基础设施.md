# 部署基础设施

<cite>
**本文档引用的文件**
- [DEPLOY.md](file://deploy/DEPLOY.md)
- [deploy.sh](file://deploy/deploy.sh)
- [nginx.conf](file://deploy/nginx.conf)
- [ecosystem.config.js](file://ecosystem.config.js)
- [package.json](file://package.json)
- [web/server/package.json](file://web/server/package.json)
- [mcp-server/package.json](file://mcp-server/package.json)
- [web/client/package.json](file://web/client/package.json)
- [config/default.ts](file://config/default.ts)
- [web/server/nodemon.json](file://web/server/nodemon.json)
- [mcp-server/src/index.ts](file://mcp-server/src/index.ts)
- [web/server/src/index.ts](file://web/server/src/index.ts)
- [web/server/src/routes/auth.ts](file://web/server/src/routes/auth.ts)
- [web/server/src/services/publisher.ts](file://web/server/src/services/publisher.ts)
- [web/server/src/middleware/auth.ts](file://web/server/src/middleware/auth.ts)
</cite>

## 更新摘要
**变更内容**
- 新增完整的自动化部署系统，包含部署指南、脚本和配置文件
- 更新 PM2 进程管理配置和启动流程
- 完善 Nginx 生产环境配置和反向代理设置
- 增强 AI MCP 服务器集成和部署流程

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

ClawOperations 是一个专门用于抖音营销账号自动化的运营系统。该系统采用前后端分离架构，包含前端 React 应用、后端 Node.js 服务、AI 内容创作 MCP 服务器以及完整的部署基础设施。

**更新** 新增了完整的自动化部署系统，提供一键部署、进程管理和生产环境配置支持。

## 项目结构

该项目采用模块化组织方式，主要包含以下核心目录：

```mermaid
graph TB
subgraph "根目录"
A[config/] 配置文件
B[deploy/] 部署脚本和配置
C[mcp-server/] AI MCP 服务器
D[src/] 核心业务逻辑
E[tests/] 测试文件
F[web/] 前后端代码
G[*.json 配置文件]
end
subgraph "web/ 目录"
H[client/] 前端应用
I[server/] 后端服务
end
subgraph "web/client/"
J[vite.config.ts] 构建配置
K[package.json] 前端依赖
L[public/] 静态资源
M[src/] 源代码
end
subgraph "web/server/"
N[index.ts] 服务器入口
O[routes/] API 路由
P[services/] 业务服务
Q[middleware/] 中间件
R[database/] 数据库
S[uploads/] 文件上传
end
subgraph "deploy/"
T[DEPLOY.md] 部署指南
U[deploy.sh] 自动化脚本
V[nginx.conf] Nginx配置
W[ecosystem.config.js] PM2配置
end
```

**图表来源**
- [package.json:1-39](file://package.json#L1-L39)
- [web/server/package.json:1-34](file://web/server/package.json#L1-L34)
- [web/client/package.json:1-32](file://web/client/package.json#L1-L32)
- [deploy/DEPLOY.md:42-55](file://deploy/DEPLOY.md#L42-L55)

**章节来源**
- [package.json:1-39](file://package.json#L1-L39)
- [web/server/package.json:1-34](file://web/server/package.json#L1-L34)
- [web/client/package.json:1-32](file://web/client/package.json#L1-L32)
- [deploy/DEPLOY.md:42-55](file://deploy/DEPLOY.md#L42-L55)

## 核心组件

### 部署管理系统

系统提供了完整的自动化部署解决方案，包含以下关键组件：

| 组件 | 版本 | 用途 | 端口 |
|------|------|------|------|
| Node.js | >=18.0.0 | 运行时环境 | 3001 |
| PM2 | 最新版 | 进程管理器 | 自动管理 |
| Nginx | 最新版 | 反向代理 | 80 |
| TypeScript | ^5.3.0 | 编译器 | 构建工具 |
| React | ^18.2.0 | 前端框架 | 前端应用 |

### 服务器配置

```mermaid
flowchart TD
A[服务器启动] --> B[检查依赖]
B --> C{Node.js安装?}
C --> |否| D[安装 Node.js 20.x]
C --> |是| E[检查 PM2]
D --> E
E --> F{PM2安装?}
F --> |否| G[安装 PM2]
F --> |是| H[检查 Nginx]
G --> H
H --> I{Nginx安装?}
I --> |否| J[安装 Nginx]
I --> |是| K[创建部署目录]
J --> K
K --> L[构建后端]
L --> M[构建前端]
M --> N[配置 Nginx]
N --> O[启动 PM2 服务]
```

**图表来源**
- [deploy.sh:30-47](file://deploy/deploy.sh#L30-L47)
- [deploy.sh:67-90](file://deploy/deploy.sh#L67-L90)

**章节来源**
- [deploy.sh:1-128](file://deploy/deploy.sh#L1-L128)
- [DEPLOY.md:108-119](file://DEPLOY.md#L108-L119)

## 架构概览

系统采用三层架构设计，包含前端、后端和 AI 服务三个主要层次：

```mermaid
graph TB
subgraph "客户端层"
A[浏览器] --> B[React 前端应用]
end
subgraph "应用层"
C[Nginx 反向代理] --> D[Node.js API 服务器]
D --> E[AI MCP 服务器]
end
subgraph "数据层"
F[本地数据库]
G[抖音 API]
H[文件存储]
end
B --> C
D --> F
D --> G
E --> D
E --> G
D --> H
```

**图表来源**
- [nginx.conf:10-49](file://deploy/nginx.conf#L10-L49)
- [web/server/src/index.ts:20-36](file://web/server/src/index.ts#L20-L36)
- [mcp-server/src/index.ts:11-21](file://mcp-server/src/index.ts#L11-L21)

## 详细组件分析

### Nginx 反向代理配置

Nginx 作为系统的统一入口，负责静态文件服务和 API 反向代理：

```mermaid
sequenceDiagram
participant Client as 客户端
participant Nginx as Nginx 代理
participant Frontend as 前端应用
participant Backend as 后端 API
participant AI as AI 服务
Client->>Nginx : 请求 /api/*
Nginx->>Backend : 反向代理到 127.0.0.1 : 3001
Backend-->>Nginx : 返回 API 响应
Nginx-->>Client : 转发响应
Client->>Nginx : 请求 / (静态文件)
Nginx->>Frontend : 提供静态文件
Frontend-->>Client : 返回前端页面
```

**图表来源**
- [nginx.conf:31-49](file://deploy/nginx.conf#L31-L49)
- [nginx.conf:25-28](file://deploy/nginx.conf#L25-L28)

### PM2 进程管理

PM2 作为进程管理器，确保后端服务的高可用性：

```mermaid
flowchart LR
A[PM2 启动] --> B[加载 ecosystem.config.js]
B --> C[启动 Node.js 服务]
C --> D[监控进程状态]
D --> E{进程崩溃?}
E --> |是| F[自动重启]
E --> |否| G[正常运行]
F --> D
```

**图表来源**
- [ecosystem.config.js:1-22](file://ecosystem.config.js#L1-L22)

### 部署流程自动化

部署脚本实现了完整的自动化部署流程：

```mermaid
flowchart TD
A[开始部署] --> B[检查 root 权限]
B --> C[安装系统依赖]
C --> D[创建部署目录]
D --> E[检查代码状态]
E --> F[安装后端依赖]
F --> G[构建后端]
G --> H[安装前端依赖]
H --> I[构建前端]
I --> J[配置 Nginx]
J --> K[启动 PM2 服务]
K --> L[部署完成]
```

**图表来源**
- [deploy.sh:24-28](file://deploy/deploy.sh#L24-L28)
- [deploy.sh:30-47](file://deploy/deploy.sh#L30-L47)
- [deploy.sh:103-115](file://deploy/deploy.sh#L103-L115)

**章节来源**
- [deploy.sh:1-128](file://deploy/deploy.sh#L1-L128)
- [nginx.conf:1-71](file://deploy/nginx.conf#L1-L71)
- [ecosystem.config.js:1-22](file://ecosystem.config.js#L1-L22)

### AI MCP 服务器集成

MCP 服务器提供 AI 内容创作能力，通过标准化接口与主系统集成：

```mermaid
classDiagram
class ClawOperationsMCP {
+string name
+string version
+tools : Tool[]
+executeTool(name, args) Promise~string~
+main() void
}
class Tool {
+string name
+string description
+inputSchema Schema
}
class APIClient {
+string baseURL
+post(endpoint, data) Promise~Response~
+get(endpoint) Promise~Response~
}
ClawOperationsMCP --> Tool : "定义工具"
ClawOperationsMCP --> APIClient : "调用 API"
APIClient --> "主系统 API" : "HTTP 请求"
```

**图表来源**
- [mcp-server/src/index.ts:23-173](file://mcp-server/src/index.ts#L23-L173)
- [mcp-server/src/index.ts:175-315](file://mcp-server/src/index.ts#L175-L315)

**章节来源**
- [mcp-server/src/index.ts:1-358](file://mcp-server/src/index.ts#L1-L358)

## 依赖关系分析

系统各组件之间的依赖关系如下：

```mermaid
graph TB
subgraph "前端依赖"
A[React ^18.2.0]
B[Ant Design ^6.3.3]
C[Axios ^1.13.6]
D[Day.js ^1.11.20]
end
subgraph "后端依赖"
E[Express ^4.18.2]
F[CORS ^2.8.5]
G[JSON Web Token ^9.0.3]
H[BcryptJS ^3.0.3]
I[LowDB ^1.0.0]
J[Multer ^1.4.5]
end
subgraph "开发依赖"
K[TypeScript ^5.3.0]
L[Jest ^29.7.0]
M[Vite ^4.4.5]
N[Nodemon ^3.0.2]
end
A --> E
E --> F
F --> G
G --> H
H --> I
I --> J
```

**图表来源**
- [web/client/package.json:12-18](file://web/client/package.json#L12-L18)
- [web/server/package.json:12-20](file://web/server/package.json#L12-L20)
- [package.json:18-34](file://package.json#L18-L34)

**章节来源**
- [web/client/package.json:1-32](file://web/client/package.json#L1-L32)
- [web/server/package.json:1-34](file://web/server/package.json#L1-L34)
- [package.json:1-39](file://package.json#L1-L39)

## 性能考虑

### 配置优化

系统在多个层面进行了性能优化：

| 优化项 | 配置 | 作用 |
|--------|------|------|
| Gzip 压缩 | 开启 | 减少传输体积 |
| 静态资源缓存 | 1年 | 提升加载速度 |
| 上传文件限制 | 500MB | 支持大文件上传 |
| 超时设置 | 60-120秒 | 防止连接超时 |
| 进程内存限制 | 500MB | 防止内存泄漏 |

### 并发处理

系统支持多实例部署和负载均衡：

```mermaid
flowchart LR
A[客户端请求] --> B[Nginx 负载均衡]
B --> C[实例 1]
B --> D[实例 2]
B --> E[实例 3]
C --> F[处理请求]
D --> F
E --> F
F --> G[返回响应]
```

**章节来源**
- [nginx.conf:18-24](file://deploy/nginx.conf#L18-L24)
- [nginx.conf:42-49](file://deploy/nginx.conf#L42-L49)
- [ecosystem.config.js:8-11](file://ecosystem.config.js#L8-L11)

## 故障排除指南

### 常见问题及解决方案

| 问题类型 | 症状 | 解决方案 |
|----------|------|----------|
| 服务未启动 | pm2 status 显示 stopped | 检查日志: pm2 logs clawops-server |
| Nginx 502 错误 | 反向代理失败 | 检查后端服务: curl http://127.0.0.1:3001/api/health |
| 抖音授权失败 | OAuth 回调错误 | 检查回调地址配置和权限范围 |
| 前端无法访问 | 404 错误 | 确认静态文件路径和 Nginx 配置 |

### 调试命令

```bash
# 查看服务状态
pm2 status

# 查看详细日志
pm2 logs clawops-server --lines 100

# 检查 Nginx 配置
nginx -t

# 重启服务
pm2 restart clawops-server
```

**章节来源**
- [DEPLOY.md:121-138](file://DEPLOY.md#L121-L138)
- [deploy.sh:124-127](file://deploy/deploy.sh#L124-L127)

## 结论

ClawOperations 的部署基础设施采用了现代化的技术栈和最佳实践，提供了完整的自动化部署、监控和运维能力。系统具备以下优势：

1. **高度自动化**：完整的部署脚本减少了人工干预
2. **高可用性**：PM2 进程管理和 Nginx 负载均衡确保服务稳定
3. **可扩展性**：模块化设计支持功能扩展和性能优化
4. **易维护性**：清晰的配置文件和日志系统便于故障排查

**更新** 新增的部署基础设施系统进一步增强了系统的生产环境支持，提供了从开发到生产的完整部署流程，确保系统能够在各种环境下稳定运行。

该基础设施为抖音营销账号的自动化运营提供了坚实的技术基础，能够满足生产环境的各种需求。