...
- [web/server/tsconfig.json](file://web/server/tsconfig.json)
- [web/client/tsconfig.json](file://web/client/tsconfig.json)
- [web/client/tsconfig.node.json](file://web/client/tsconfig.node.json)
...
- [web/server/.env.example](file://web/server/.env.example)
</cite>
...
**新增** 项目现已包含完整的 Web 界面系统，提供图形化界面用于配置认证、上传视频和管理发布任务。系统采用 MySQL 数据库存储用户数据、认证配置、创作任务和应用配置，同时使用 Redis 缓存系统配置以提高性能。

## 项目结构
- 核心源码位于 src/，按职责划分为 api、services、utils、models、config 等目录
- 测试位于 tests/，按功能划分单元测试
- Web 界面位于 web/ 目录，包含客户端 React 应用和服务器端 API 服务
- 数据库层位于 web/server/src/database/，包含连接管理、迁移脚本和表结构定义
...
I["index.ts<br/>数据库连接管理"]
J["schema.sql<br/>表结构定义"]
K["migrate-from-lowdb.ts<br/>数据迁移脚本"]
end
subgraph "测试(tests)"
T1["unit/"]
end
...
**更新** 服务器端包配置已更新 main 和 start 脚本指向新的 dist/web/server/src/index.js 路径

**章节来源**
- [package.json:13-16](file://package.json#L13-L16)
- [web/client/package.json:6-11](file://web/client/package.json#L6-L11)
- [web/server/package.json:5-11](file://web/server/package.json#L5-L11)

### TypeScript 配置更新

#### 根目录 TypeScript 配置
根目录的 tsconfig.json 已扩展 include 路径并简化 baseUrl 设置：
...
#### Web 服务器 TypeScript 配置
Web 服务器的 tsconfig.json 已扩展 include 路径并简化 baseUrl 设置：
...
**更新** TypeScript 配置已扩展 include 路径并简化 baseUrl 设置

**章节来源**
- [tsconfig.json:1-20](file://tsconfig.json#L1-L20)
- [web/server/tsconfig.json:1-20](file://web/server/tsconfig.json#L1-L20)
...
### 数据库连接配置
系统采用 MySQL 作为主要数据存储，Redis 作为缓存层，提供高可用的数据管理能力。

#### 数据库连接管理
数据库连接通过统一的连接池管理，支持：
- MySQL 连接池：最大连接数 10，超时重试机制
- Redis 连接：支持重连策略，最大重试 3 次
- 自动初始化：启动时自动创建数据库连接和 Redis 客户端

#### 环境变量配置
服务器端需要以下数据库相关环境变量：
- `DB_HOST`: MySQL 主机地址，默认 127.0.0.1
- `DB_PORT`: MySQL 端口，默认 3306
- `DB_USER`: MySQL 用户名，默认 clawops
- `DB_PASS`: MySQL 密码，默认 ClawOps@2024!
- `DB_NAME`: MySQL 数据库名，默认 clawops
...
- [web/server/.env.example:8-16](file://web/server/.env.example#L8-L16)
...
存储系统用户信息，支持抖音第三方登录：
- 主键：自增 ID
- 唯一约束：用户名、邮箱、抖音 OpenID
- 支持字段：用户名、邮箱、密码哈希、手机号、头像、角色、状态

#### 用户认证配置表 (user_auth_configs)
存储用户的抖音认证配置：
- 外键约束：user_id 引用 users.id，级联删除
- 支持字段：客户端密钥、客户端密钥、重定向URI、访问令牌、刷新令牌、OpenID、过期时间

#### 创作任务表 (creation_tasks)
存储 AI 创作任务的草稿和历史记录：
- 支持字段：任务类型（草稿/历史）、状态、需求、内容类型、分析结果、内容、文案、进度等

#### 创作模板表 (creation_templates)
存储 AI 创作模板：
- 支持字段：模板名称、描述、需求、内容类型、标签、参考图片等

#### 应用配置表 (app_config)
存储系统配置信息：
- 主键：配置键
- 支持字段：AI 配置、抖音配置等 JSON 格式配置

**章节来源**
- [web/server/src/database/schema.sql:4-79](file://web/server/src/database/schema.sql#L4-L79)

### 数据迁移流程
系统提供从低版本数据库到 MySQL 的迁移脚本：

#### 迁移脚本功能
- 读取旧版 db.json 数据
- 连接 MySQL 数据库
- 逐条迁移用户、认证配置、创作任务、模板和应用配置
- 支持事务回滚，确保数据一致性

#### 迁移前置条件
1. 已在 MySQL 中执行 schema.sql 创建表结构
2. .env 已配置数据库连接参数
3. db.json 文件存在且格式正确

#### 迁移执行方式
- 命令行：`npm run migrate`
- 直接执行：`ts-node src/database/migrate-from-lowdb.ts`
...
#### 缓存层次
1. **内存缓存**：SystemConfigService 内部缓存，同步读取
2. **Redis 缓存**：异步写入，5分钟TTL
3. **数据库缓存**：MySQL 持久化存储

#### 配置同步
- 启动时从数据库加载到内存缓存
- 写入时同步更新数据库和 Redis 缓存
- 环境变量同步：敏感配置同步到进程环境变量
...
#### 用户操作
- 用户创建：验证用户名、邮箱格式，密码强度
- 用户查询：支持按用户名、邮箱、ID、抖音 OpenID 查询
- 用户更新：支持字段级更新，自动去重检查
- 密码管理：bcrypt 哈希加密，密码强度验证

#### 抖音第三方登录
- 支持通过抖音 OpenID 创建或更新用户
- 自动同步抖音用户信息（昵称、头像等）
- 生成随机用户名和临时密码

**章节来源**
- [web/server/src/services/user-service.ts:39-264](file://web/server/src/services/user-service.ts#L39-L264)

## 结语
本指南围绕开发环境、构建配置、开发流程、代码规范、调试与性能、扩展与发布等方面提供了系统性说明。Web 界面系统的加入为用户提供了直观的操作界面，结合原有的 SDK 功能，形成了完整的抖音内容管理解决方案。

**更新摘要**
- 新增数据库架构章节，详细说明 MySQL 和 Redis 的配置与使用
- 更新了数据库连接配置说明，包括环境变量设置和连接池管理
- 增强了数据迁移流程说明，提供从低版本数据库到 MySQL 的完整迁移方案
- 移除了文件存储相关的开发配置，反映项目向纯数据库架构的转变
- 更新了系统配置服务的说明，强调多层缓存机制和配置同步策略
- 增强了用户数据管理的说明，包括第三方登录和数据验证机制

建议在团队内形成文档与流程共识，持续完善测试与监控体系，保障系统的稳定性与可维护性。