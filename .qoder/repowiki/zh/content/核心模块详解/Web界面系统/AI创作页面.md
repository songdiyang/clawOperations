...
**更新** 新版本引入了重新生成文案功能，用户可以通过质量校验反馈自动优化内容，显著提升了内容质量和创作效率。
...
- [ai.ts:1-1081](file://web/server/src/routes/ai.ts#L1-L1081)
...
Router->>Service : executeStep('copywriting')
Service->>AI : DeepSeek重新生成文案
AI-->>Service : 返回优化后的文案
Service-->>Router : 返回优化结果
Router-->>API : 返回优化结果
API-->>UI : 显示优化后的文案
```

**架构图来源**
- [AICreator.tsx:176-233](file://web/client/src/pages/AICreator.tsx#L176-L233)
- [ai.ts:139-164](file://web/server/src/routes/ai.ts#L139-L164)
...
QualityCheck --> RegenerateButton[重新生成按钮]
```

**更新** 新增重新生成按钮和增强的质量校验组件

**架构图来源**
- [AICreator.tsx:216-715](file://web/client/src/pages/AICreator.tsx#L216-L715)
...
**架构图来源**
- [ai.ts:96-1081](file://web/server/src/routes/ai.ts#L96-L1081)
