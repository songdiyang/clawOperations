...
subgraph "数据库层"
J[web/server/src/database/ - 数据库管理]
K[web/server/src/models/ - 数据模型]
end
A --> B
A --> C
B --> F
...
### 用户认证配置服务 - UserAuthConfigService

**更新** 新增了专门的用户认证配置服务，负责管理用户的抖音认证配置，确保令牌保存时包含完整的应用配置数据。
...
- [web/server/src/models/user.ts:98-110](file://web/server/src/models/user.ts#L98-L110)
...
Auth[认证模块]
UserAuthConfigService[用户认证配置服务]
...
AuthRoutes --> UserAuthConfigService
AuthRoutes --> AppConfigService
...
UserAuthConfigService --> Database
...
**更新** 新增了完整的数据库和配置管理机制，确保令牌配置的完整性和可靠性。

```mermaid
graph LR
subgraph "数据库结构"
A[user_auth_configs - 用户认证配置表]
B[users - 用户表]
C[app_config - 应用配置表]
D[_meta - 元数据表]
end
subgraph "配置服务"
E[UserAuthConfigService - 用户认证配置服务]
F[AppConfigService - 应用配置服务]
end
subgraph "认证流程"
G[OAuth回调处理]
H[令牌保存]
...
**图表来源**
- [web/server/src/database/index.ts:9-36](file://web/server/src/database/index.ts#L9-L36)
...
- [web/server/src/services/app-config-service.ts:28-58](file://web/server/src/services/app-config-service.ts#L28-L58)
...
### 数据持久化优化
**更新** 新增了高效的数据库操作和配置缓存机制，确保令牌配置的快速访问和持久化存储。
...
- **令牌过期**：实现自动刷新机制
- **权限不足**：确认OAuth作用域配置
- **令牌保存不完整**：检查用户认证配置服务的upsertConfig方法
...
**更新** 新增了配置管理相关的故障排除指南：
- **配置丢失**：检查数据库连接和表结构
- **令牌不完整**：验证UserAuthConfigService的配置保存逻辑
- **权限冲突**：确认用户认证配置与全局配置的一致性
...
- **可扩展性**：插件化的架构支持功能扩展
- **易用性**：简洁的API接口降低使用门槛
- **数据完整性**：新增的用户认证配置服务确保令牌保存时包含完整的应用配置数据
- **可靠性**：增强的令牌配置管理和持久化机制
...
### 最新改进
**更新** 系统最近进行了重要改进，修复了OAuth令牌保存时缺少应用配置数据的问题，通过以下方式增强了系统的完整性和可靠性：
