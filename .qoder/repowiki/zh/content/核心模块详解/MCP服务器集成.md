...
| publish_video | 发布视频到抖音平台，支持立即发布和定时发布 | videoPath, title, description, hashtags, publishTime, isRemoteUrl | 发布结果、视频ID |
| get_publish_tasks | 获取所有定时发布任务列表 | 无 | 任务列表 |
| cancel_publish_task | 取消指定的定时发布任务 | taskId | 取消结果 |
| get_auth_status | 获取抖音认证状态 | 无 | 认证状态 |
| ai_create_and_publish | AI一键创作并发布：完整工作流 | requirement, contentType, scheduleTime | 完整工作流结果 |
