...
- [doubao-client.d.ts](file://api/ai/doubao-client.d.ts)
</cite>
...
- **更新**：修复文件输出路径一致性问题，将输出目录配置从使用 process.cwd() 改为 __dirname 基础的路径解析系统
...
- **文件输出路径一致性**：修复了跨环境部署中文件保存位置不一致的问题
...
**图表来源**
- [doubao-client.ts:1-407](file://src/api/ai/doubao-client.ts#L1-L407)
...
- **文件输出路径一致性**：使用 __dirname 基础的路径解析系统，确保生成的文件始终保存到项目根目录的 /generated/ 目录中
...
- **跨环境部署兼容性**：修复了工作目录差异导致的文件保存位置不一致问题
...
**章节来源**
- [doubao-client.ts:94-407](file://src/api/ai/doubao-client.ts#L94-L407)
...
DoubaoClient 现在使用基于 __dirname 的路径解析系统来确保文件输出路径的一致性：

```mermaid
flowchart TD
Start([DoubaoClient 初始化]) --> GetDir[__dirname 获取]
GetDir --> CalcPath[计算相对路径]
CalcPath --> JoinPath[path.join(__dirname, '../../../generated')]
JoinPath --> CheckDir{检查目录存在}
CheckDir --> |不存在| CreateDir[创建目录]
CheckDir --> |存在| UseDir[使用现有目录]
CreateDir --> UseDir
UseDir --> StoreFiles[存储生成文件]
StoreFiles --> ConsistentPaths[确保路径一致性]
```

**图表来源**
- [doubao-client.ts:122-127](file://src/api/ai/doubao-client.ts#L122-L127)

**更新说明**：
- **原实现问题**：使用 `process.cwd()` 会导致在不同执行环境下工作目录不同，从而影响文件保存位置
- **新实现方案**：使用 `__dirname` 作为基准，通过 `path.join(__dirname, '../../../generated')` 确保始终指向项目根目录下的 `/generated/` 目录
- **跨环境兼容性**：无论在开发环境还是生产环境，文件都会被保存到相同的绝对路径位置
- **部署一致性**：解决了生产环境中工作目录差异导致的文件保存位置不一致问题
...
- **路径一致性保证**：使用 __dirname 基础的路径解析系统，确保跨环境部署的一致性
...
- **文件路径缓存**：输出目录路径在客户端实例初始化时确定，避免重复计算
...
**解决方法：**
1. **确认修复**：DoubaoClient 现在使用基于 __dirname 的路径解析系统
2. 检查项目根目录下是否存在 `/generated/` 目录
3. 验证文件权限设置
4. 确认静态文件服务配置正确
5. **新增**：检查服务器启动时的工作目录是否正确
...
- **跨环境部署兼容性**：修复了文件输出路径一致性问题，增强了生产环境稳定性
...
- **文件路径一致性**：使用 __dirname 基础的路径解析系统，确保生成的文件始终保存到项目根目录的 /generated/ 目录中
- **生产环境稳定性**：解决了工作目录差异导致的文件保存位置不一致问题
...
- **多环境部署**：支持开发、测试、生产等不同环境的一致性

该系统为开发者提供了一个可靠的AI内容生成基础框架，可以根据具体需求进行定制和扩展。新增的参考图像支持功能、增强的错误处理机制以及文件输出路径一致性修复进一步提升了系统的实用性和可靠性，使其能够更好地满足现代内容营销的需求。